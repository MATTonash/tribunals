...
expTaskArray.forEach(element => {
  ...
  promises.push(getItems(expComp.src, pageNum))
...
await Promise.all(promises).then(dt => {
  console.log(dt);
  dataPageArray = dt;
})
...
dataPageArray.forEach((element, index) => {
  let expComp = expTaskArray[index]
  ...
  contentHighlight.push(taskArray)
})
